...
if(isset($_GET["action"])&&($_GET["action"]=="delete")){
  $query_delMember = "DELETE FROM memberdata WHERE m_id=?";
	...
	$stmt->close();
	//重新導向回到主畫面
	header("Location: m_admin2.php");
}
//回復會員
if(isset($_GET["action"])&&($_GET["action"]=="reboot")){
  $query_rebMember = "UPDATE memberdata SET m_valid=1 WHERE m_id=?";
	$stmt=$db_link->prepare($query_rebMember);
	$stmt->bind_param("i", $_GET["id"]);
	$stmt->execute();
	$stmt->close();
	//重新導向回到主畫面
...
//未加限制顯示筆數的SQL敘述句
$query_RecMember = "SELECT * FROM memberdata WHERE m_level<>'admin' && m_valid=0 ORDER BY m_jointime DESC";
  ...
  <?php }?>
    ...
    <nav aria-label="breadcrumb">
      <ol class="breadcrumb">
        ...
        <li class="breadcrumb-item active" aria-current="page">已停用會員</li>
      </ol>
    </nav>
    <!-- main  -->
    <div class="container mb-4" style="min-height:561px">
     ...
     <div class=" mt-2 pb-3">
        ...
        <ul class="nav nav-tabs">
          ...
          <li class="nav-item">
            <a class="nav-link active " href="#">已停用會員</a>
          </li>
          <li class="nav-item">
        ...
        <table class="table table-hover table-bordered ">
          ...
          ?>
            ...
            <tr>
              ...
              <td align="center"> 
                <a type="button" class="btn btn-outline-success ft2" href="?action=reboot&id=<?php echo $row_RecMember["m_id"];?>"
                onClick="return rebootsure();">
                <i class="bi bi-arrow-repeat"></i> 回復
                </a>
